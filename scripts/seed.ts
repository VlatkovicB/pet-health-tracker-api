...
async function main() {
  const sequelize = new Sequelize({
    ...
    logging:  false,
    models: [
      UserModel, PetModel, VetModel, VetVisitModel,
      MedicationModel, ReminderModel, ReminderNotifyUserModel,
    ],
  });
  ...
  for (let i = 0; i < VETS.length; i++) {
    await VetModel.create({ id: vetIds[i], userId, ...VETS[i], createdAt: yearsAgo(2) });
  }
  ...
  for (let pi = 0; pi < PETS.length; pi++) {
    ...
    await PetModel.create({ id: petId, userId, ...p, createdAt: yearsAgo(2) });
